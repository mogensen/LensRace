...
- `src/i18n/` — `vue-i18n` setup. UI strings are in English and Danish (`locales/en.ts`, `locales/da.ts`); the locale is picked automatically from the browser's `navigator.languages`, falling back to English. Category/item names and server-side error messages come from the backend and aren't translated.
